...
| 🖼️ **Very high-resolution wallpapers fail to decode — including a blank/black lock screen on suspend** | Qt's default image allocation limit is 256 MiB, and it applies separately to every process that decodes the wallpaper (the shell, AND the lock screen surface — fixing one doesn't fix the other). Symptoms: `QImageIOHandler: Rejecting image... exceeds allocation limit`, `colors.json generation failed`, or the lock screen just showing black/gray where the wallpaper should be, with `[LockSurface] Image source load error` in `inir logs`. Fix at the source with `environment { QT_IMAGEIO_MAXALLOC "0"; }` in `config.kdl` (`0` = no limit, already set in this repo's `config.kdl`) — or resize the offending wallpaper: `magick file.jpg -resize 3840x2160\> -quality 90 file.jpg`. |
